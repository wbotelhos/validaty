...
;(function($) {
  ...
  methods._balloon = function(result) {
    ...
    var html       = '<ul></ul>';
    var attributes = { id: result.el[0].hash, 'class': 'validaty-message' };
  ...
  methods._display = function(result) {
    var target = methods._balloon.call(this, result);

    methods._writeMessage.call(this, target, result);
    methods._showMessage.call(this, target);
  };
  ...
  methods._showMessage = function(target) {
  ...
  methods._validate = function(input) {
    ...
    for (var i = 0; i < validations.length; i++) {
      ...
      var message = methods._message.call(that, input, validator, args);
  ...
  methods._writeMessage = function(target, result) {
    var wrapper = target.children('ul');

    $.each(result.fail, function() {
      $('<li />', { text: this.message }).appendTo(wrapper);
    });
  };
