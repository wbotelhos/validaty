...
;(function($) {
  ...
  methods._distinct = function(fields) {
    var names    = [];
    var distinct = [];
    var inputs   = this.opt.balloon ? fields : $($(fields).get().reverse());
  ...
  methods._fade = function() {
    var that     = this;
    var balloons = $(that).children(that.opt.balloon ? '.validaty-balloon' : '.validaty-message');

    if (balloons.length > 1) {
      balloons.on('mouseenter.validaty', function() {
        var overed = this;
        var other  = balloons.filter(function() { return this !== overed; });
  ...
  methods._process = function(fields, evt) {
    ...
    } else if (this.opt.onFail) {
      this.opt.onFail.call(this, fields, evt);
    }
  };
